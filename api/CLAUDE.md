...
This backend is paired with the React admin dashboard + embeddable widget at the sibling app:

    ../admin   (sibling at monorepo root)

That app has its own CLAUDE.md describing its stack (React 18 + Vite 6 + TanStack Query + Tailwind v4) and conventions.

Rules for any frontend work:
- DO NOT edit frontend files from this backend session.
- Spawn a dedicated agent (via the Agent tool) that operates WITH cwd inside `../admin` (relative to this app), reads that app's CLAUDE.md first, invokes its `dev-guideline` skill, and abides by every rule there.

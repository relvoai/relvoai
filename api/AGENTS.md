...
MANDATORY — before doing ANY work on this project, invoke the `dev-guideline` skill.

Rules:
- At the start of every new user request that involves code, tests, migrations, routes, controllers, models, or config, invoke `dev-guideline` BEFORE any other tool call.
- At task completion (before summarizing to the user), invoke `dev-guideline` again for the self-review gate.
- When working with backend APIs, invoke `dev-guideline` for its backend-integration guidance.
...
=== laravel boost mcp enforcement ===

MANDATORY — Laravel Boost MCP is the primary source of truth for Laravel ecosystem knowledge on this project.

Use it for:
- **Documentation**: `mcp__laravel-boost__search-docs` BEFORE writing code for Laravel, Sanctum, Reverb, Broadcasting, Notifications, Prism (AI SDK), Pest, Telescope, Laratrust, Scramble, Tailwind, or any installed package. Do not rely on training memory for package APIs — always verify via Boost first, since it returns version-specific docs pinned to this app's installed versions.
- **Schema lookup**: `mcp__laravel-boost__database-schema` / `mcp__laravel-boost__database-query` instead of opening migration files by hand.
- **Artisan**: `mcp__laravel-boost__list-artisan-commands` before invoking any artisan command to confirm flags.
- **Config + env**: `mcp__laravel-boost__get-config` / `mcp__laravel-boost__list-available-config-keys` / `mcp__laravel-boost__list-available-env-vars`.
- **Routes**: `mcp__laravel-boost__list-routes` for auditing endpoints.
- **Debugging**: `mcp__laravel-boost__tinker` / `mcp__laravel-boost__read-log-entries` / `mcp__laravel-boost__last-error` / `mcp__laravel-boost__browser-logs`.
- **URL generation**: `mcp__laravel-boost__get-absolute-url` — never hand-build URLs for Herd-served apps.

Rules:
- Before writing or refactoring ANY Laravel code, run at least one `search-docs` query relevant to the change. Pass an array of broad, topic-based queries.
- Do not duplicate your own searches — if you already have the answer in this conversation, proceed.
- If Boost is unreachable, report it and stop — do not fall back to memory for Laravel APIs.
- Do NOT pass package names as query terms (Boost already filters by installed versions).

This rule works in parallel with `dev-guideline` — invoke `dev-guideline` first for lifecycle, then use Boost throughout implementation.

=== paired frontend repo ===

This backend is paired with a React admin dashboard + embeddable widget at:

    /Users/benny/Documents/react/relvoai-admin

That repo has its own CLAUDE.md describing its stack (React 18 + Vite 6 + TanStack Query + Tailwind v4) and conventions.

Rules for any frontend work:
- DO NOT edit frontend files from this backend session.
- Spawn a dedicated agent (via the Agent tool) that operates WITH cwd inside `/Users/benny/Documents/react/relvoai-admin`, reads that repo's CLAUDE.md first, invokes its `dev-guideline` skill, and abides by every rule there.
- In the agent prompt, explicitly list: the task scope, the backend endpoints/contracts involved, the Reverb channel names, and the auth flow (Sanctum Bearer).
- For coordinated full-stack changes: land the backend change + contract here first, then brief the frontend agent with the finalized shape.
- Never let the frontend agent call backend files; if it needs a backend change, have it report back so we land that change in this repo.
